...
test("เชื่อม LINE ด้วยรหัส 6 ตัวผ่าน webhook แล้ว cron สแกนงานค้าง → ส่ง push (dry-run)", async ({ page }) => {
  ...
  const body = JSON.stringify({
    destination: "Uoa",
    events: [{ type: "message", replyToken: "r1", source: { type: "user", userId: lineUserId }, message: { type: "text", text: code.toLowerCase() } }],
  });
  const bad = await page.request.post("/api/line/webhook", { headers: { "x-line-signature": "nope", "content-type": "application/json" }, data: body });
  expect(bad.status()).toBe(401);
  const ok = await page.request.post("/api/line/webhook", { headers: { "x-line-signature": sign(body), "content-type": "application/json" }, data: body });
  ...
  const scan = await page.request.post("/api/cron/scan-overdue", { headers: { authorization: `Bearer ${CRON_SECRET}` } });
  ...
  let sent = 0;
  for (let i = 0; i < 40; i++) {
    const processed = await page.request.post("/api/cron/process-events", { headers: { authorization: `Bearer ${CRON_SECRET}` } });
    expect(processed.status()).toBe(200);
    const summary = (await processed.json()) as { fetched: number; processed: number; sent: number; failed: number };
    sent += summary.sent;
