...
describe("computeProgress (POC Decisions 1.1-1.2)", () => {
  it("metric = current/target ไม่สน task", () => {
    ...
    expect(
      computeProgress(
        goal({ id: "m2", goal_kind: "metric", target_value: 100, current_value: 250 }),
        [],
        [],
      ),
    ).toBe(100);
    expect(
      computeProgress(goal({ id: "m3", goal_kind: "metric", target_value: null }), [], []),
    ).toBe(0);
  });
...
describe("paceStatus / isMetricComplete", () => {
  ...
  it("metric สำเร็จเมื่อถึงเป้า", () => {
    expect(isMetricComplete({ goal_kind: "metric", target_value: 10, current_value: 10 })).toBe(
      true,
    );
    expect(isMetricComplete({ goal_kind: "metric", target_value: 10, current_value: 9.99 })).toBe(
      false,
    );
    expect(isMetricComplete({ goal_kind: "execution", target_value: null, current_value: 0 })).toBe(
      false,
    );
  });
});
