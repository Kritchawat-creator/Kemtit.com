...
export async function createTask(input: unknown): Promise<ActionResult<{ id: string }>> {
  ...
  if (parsed.data.goalId && !(await ensureOwnGoal(supabase, parsed.data.goalId))) return fail("invalidGoal");

  const { data, error } = await supabase.from("tasks").insert(toInsert(user.id, parsed.data)).select("id").single();
  if (error || !data) {
...
export async function updateTask(input: unknown): Promise<ActionResult<{ id: string }>> {
  ...
  void _ignored;
  const { data, error } = await supabase.from("tasks").update(patch).eq("id", id).select("id, goal_id").maybeSingle();
...
export async function deleteTask(input: unknown): Promise<ActionResult> {
  ...
  const { data, error } = await supabase.from("tasks").delete().eq("id", parsed.data.id).select("id, goal_id").maybeSingle();
  if (error) return fail("generic");
  revalidateTasks(data?.goal_id);
  return ok(null);
}
...
export async function toggleTask(
  ...
  const { data: task } = await supabase.from("tasks").select("id, goal_id, recurrence_rule").eq("id", id).maybeSingle();
  if (!task) return fail("notFound");

  if (task.recurrence_rule) {
    const { error } = done
      ? await supabase.from("task_completions").upsert({ task_id: id, user_id: user.id, completed_on: date }, { onConflict: "task_id,completed_on", ignoreDuplicates: true })
  ...
  if (done) await emitEvent(supabase, user.id, "task.completed", { taskId: id, goalId: task.goal_id, date });
  ...
  if (done && task.goal_id) {
    ...
    while (current) {
      if (current.goal_kind === "execution" && current.status === "active" && !current.completed_at && current.progress.percent >= 100) {
        const { error } = await supabase
          .from("goals")
          .update({ completed_at: new Date().toISOString(), status: "completed" })
          .eq("id", current.id);
        if (!error) {
          await emitEvent(supabase, user.id, "goal.completed", { goalId: current.id, title: current.title, periodType: current.period_type });
