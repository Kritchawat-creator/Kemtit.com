      ...
      >
        ...
        <span className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-caption text-text-secondary">
          <span className="inline-flex items-center gap-1">
            <span className={cn("size-1.5 rounded-full", DOMAIN_STYLES[task.domain].dot)} aria-hidden="true" />
            {t(`domains.${task.domain}`)}
          </span>
          {rule ? (
            <span className="inline-flex items-center gap-1">
              <Repeat className="size-3" aria-hidden="true" />
              {rule.freq === "DAILY" ? t("tasks.recurrence.badgeDaily") : t("tasks.recurrence.badgeWeekly")}
            </span>
          ) : null}
          {item.overdue ? (
            <span className="text-warning-800">{t("tasks.meta.overdueSince", { date: formatThaiDate(item.date, "short") })}</span>
          ) : item.date !== today ? (
            <span>{formatThaiDate(item.date, "weekday")}</span>
          ) : null}
          {showGoal && task.goal ? <span className="truncate">{t("tasks.meta.goal", { title: task.goal.title })}</span> : null}
        </span>
      </button>
    </li>
  );
